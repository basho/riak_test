...
create_type(TypeName, Props) ->
    call_to_node(riak_core_bucket_type, create, [TypeName, Props]).
...
activate_type(TypeName) ->
    call_to_node(riak_core_bucket_type, activate, [TypeName]).
...
update_type(TypeName, Props) ->
    call_to_node(riak_core_bucket_type, update, [TypeName, Props]).
...
%     call_to_node(riak_core_bucket_type, set_bucket, [Bucket, Props]).
...
type_status(TypeName) ->
    ...
    call_to_node(riak_core_bucket_type, status, [TypeName]).
...
% get_type(TypeName) ->
%     call_to_node(riak_core_bucket_type, get, [TypeName]).
...
call_to_node(Module, FunName, Props) ->
    [{nodes, Nodes}] = ets:lookup(?ETS, nodes),
    [Node|_] = Nodes,
    rpc:call(Node, Module, FunName, Props).
...
confirm() ->
    lager:info("Activate ECQ to run verify_bucket_types"),
    pass.
-endif.
