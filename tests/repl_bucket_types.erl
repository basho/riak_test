...
ensure_bucket_sent(Pid, Bucket, Key) ->
...
assert_bucket_found(Pid, Bucket, Key) ->
    case riakc_pb_socket:get(Pid, Bucket, Key) of
        {error, notfound} ->
	    false;
	{ok, _} ->
...
get_legacy_pb_pid(Leader) ->
    {ok, IP} = rpc:call(Leader, application, get_env, [riak_api, pb_ip]),
    {ok, PortA} = rpc:call(Leader, application, get_env, [riak_api, pb_port]),
    {ok, Pid} = riakc_pb_socket:start_link(IP, PortA, []),
    Pid.
