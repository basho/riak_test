...
sys_get_state(Pid) ->
    S = sys:get_status(Pid),
    element(2,lists:nth(1, element(2, lists:nth(3, lists:nth(5,element(4, S)))))).

%% @doc Ensure we can cancel a fullsync and restart it.
confirm() ->
    Conf = ?CONF(5),
    Nodes = [ANodes, BNodes] = rt:build_clusters([{3, Conf}, {3, Conf}]),
    ...
    States = [sys_get_state(P) || P <- Coordinators],
    KeylistPids = lists:flatten([element(14, State) || State <- States]),
    KLStates = [sys_get_state(Pid) || {Pid, _} <- KeylistPids],
