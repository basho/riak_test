...
confirm() ->
    ...
    {ok, _} = riak_ensemble_client:kput_once(Node, root, testerooni, 
        testvalue, 1000),
    lager:info("Read that value from the root ensemble"),
    ...
    ok = ensemble_util:wait_for_membership(Node),
    lager:info("Successfully read that value from the root ensemble"),
