...
setup_cluster_put_explicit(ClusterType, TestType, DDL, N, Ts) ->
    [Node | _] = timeseries_util:build_cluster(ClusterType),
    ...
    Bucket = list_to_binary(timeseries_util:get_bucket()),
    C = rt:pbc_explicit(Node),
    Data = [[<<"family1">>, <<"seriesX">>, 100, 1, <<"test1">>, 1.0, true]],
    io:format("Time before = ~p~n", [my_time()]),
    profiler:perf_profile({start, 0}),
    ok = putDataExplicit(C, Bucket, Data, N, Ts),
    profiler:perf_profile({stop,  0}),
    io:format("Time after = ~p~n", [my_time()]).
...
putDataExplicit(_C, _Bucket, _Data, _N, 0, _Ts) ->
    ok;
putDataExplicit(C, Bucket, Data, N, Acc, Ts) ->
    case Ts of
	true ->
	    ok = riakc_ts:explicit_put(C, Bucket, Data);
	false ->
	    Key = list_to_binary("key"++integer_to_list(Acc)),
	    Obj = riakc_obj:new({<<"GeoCheckin">>,<<"GeoCheckin">>}, Key, Data),
	    ok  = riakc_pb_socket:put(C, Obj)
    end,
    putDataExplicit(C, Bucket, Data, N, Acc-1, Ts).
