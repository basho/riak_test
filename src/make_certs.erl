...
test_certs() ->
    Tmp = string:strip(os:cmd("mktemp -d"), right, $\n),
    rootCA(Tmp, "rootCA"),
    intermediateCA(Tmp, "childCA", "rootCA"),
    endusers(Tmp, "childCA", ["site1.basho.com", "site2.basho.com"]),
    endusers(Tmp, "rootCA", ["site4.basho.com", "site3.basho.com"]),
    filelib:ensure_dir("ca/"),
    os:cmd("cp " ++ Tmp ++ "/childCA/cert.pem ca/cacert.pem"),
    os:cmd("cp " ++ Tmp ++ "/rootCA/cert.pem ca/rootcert.pem"),
    Sites = [{"site1", "site1.basho.com"},
             {"site2", "site2.basho.com"},
             {"site3", "site3.basho.com"},
             {"site4", "site4.basho.com"}],
    _ = [copy_site(Tmp, Name, Dir) || {Name, Dir} <- Sites],
    ok.

copy_site(Tmp, Name, Dir) ->
    Path = filename:join(Tmp, Dir),
    os:cmd("cp " ++ Path ++ "/cert.pem " ++ Name ++ "-cert.pem"),
    os:cmd("cp " ++ Path ++ "/key.pem " ++ Name ++ "-key.pem"),
    ok.
